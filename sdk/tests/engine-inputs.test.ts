...
const shieldW = (assetField?: bigint) => ({ note, commitment: 10n, ownerCommitment: 11n, assetField });
const transferW = (assetField?: bigint) => ({ nullifier: 1n, newCommitment: 2n, newOwnerCommitment: 3n, input: note, ownerSk: 9n, output: note, membership, assetField });
const splitW = (assetField?: bigint) => ({ nullifier: 1n, commitment1: 2n, ownerCommitment1: 3n, commitment2: 4n, ownerCommitment2: 5n, input: note, ownerSk: 9n, out1: note, out2: note, membership, assetField });
const mergeW = (assetField?: bigint) => ({ nullifier1: 1n, nullifier2: 2n, commitment: 3n, ownerCommitment: 4n, input1: note, ownerSk1: 9n, membership1: membership, input2: note, ownerSk2: 9n, membership2: membership, output: note, assetField });
...
describe("engine circuit routing", () => {
  for (const c of cases) {
    it(`${c.name}: native path — STX circuit, no asset_id`, () => {
    ...
    it(`${c.name}: SIP-10 path — sip10 circuit, asset_id bound`, () => {
      ...
      expect(inputs["circuit_version"]).toBe("1");
    });
  }
});
