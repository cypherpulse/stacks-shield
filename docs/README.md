...
4. **[Security](security.md)**: threat model, guarantees, invariants, and the
   trust assumptions (including the zkVerify dependency). See the
   **[security review](../audits/security-review.md)** for the findings and their remediation
   status (internal review, not an independent audit).
...
- **[Toward native ZK verification](whitepaper.md#9-toward-native-zk-verification-on-stacks)**: the path from delegated verification to fully native, on-chain proofs.
...
> **Status: live on Stacks Testnet, v2.** Internally reviewed (see the
> [security review](../audits/security-review.md)). **Not** independently audited, and
> not on mainnet. See the whitepaper's [Limitations](whitepaper.md#8-limitations).
