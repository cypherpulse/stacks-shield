// Deploys privacy-registry on its own. Part of the ordered pipeline in
...
const main = async () => {
  ...
  const txid = await d.deployContract("privacy-registry");
  console.log(txid ? `privacy-registry deployed: ${txid}` : "privacy-registry already deployed");
};
