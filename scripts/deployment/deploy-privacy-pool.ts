// Deploys privacy-pool on its own. Part of the ordered pipeline in
...
const main = async () => {
  ...
  const txid = await d.deployContract("privacy-pool");
  console.log(txid ? `privacy-pool deployed: ${txid}` : "privacy-pool already deployed");
};
